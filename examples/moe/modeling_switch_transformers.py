...
class SwitchTransformersAttention(nn.Module):
    ...
    def compute_bias(self, batch_size, query_length, key_length, device=None):
        ...
        values = self.relative_attention_bias(relative_position_bucket.unsqueeze(0).repeat(batch_size, 1, 1))  # shape (query_length, key_length, num_heads)
    ...
    def forward(
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                position_bias = self.compute_bias(batch_size, real_seq_length, key_length, device=scores.device)
